...
export default function ParticipantEvents() {
  ...
  const getStats = () => {
    ...
    const upcoming = registrations.filter((r) => {
      ...
      const hasValidStatus = ['Registered', 'Waitlisted'].includes(r.status);
      
      // For merchandise events, also check payment approval
      if (r.eventId?.type === 'Merchandise') {
        return isFuture && r.status === 'Registered' && r.paymentStatus === 'Approved';
      }
  ...
  const filteredRegistrations = registrations
    .filter((reg) => {
      ...
      switch (activeTab) {
        case 'upcoming':
          // For merchandise events, only show if payment is approved
          // For normal events, show if registered or waitlisted
          if (reg.eventId?.type === 'Merchandise') {
            return eventStart > now && reg.status === 'Registered' && reg.paymentStatus === 'Approved';
          }
              ...
              return (
                <div key={reg._id} style={styles.eventCard} onMouseOver={(e) => (e.currentTarget.style.transform = 'translateY(-4px)')} onMouseOut={(e) => (e.currentTarget.style.transform = 'translateY(0)')}>
                  ...
                  <div style={{ padding: '2rem' }}>
                    ...
                    {/* Ticket ID - Only show if QR code exists (payment approved for merchandise or normal event) */}
                    {reg.qrCode && (
                    ...
                    {/* Payment Status for Merchandise - Enhanced with Clear Instructions */}
                    {reg.eventId?.type === 'Merchandise' && (
                      <div style={{ marginBottom: '1.5rem', padding: '1.5rem', borderRadius: '1rem', border: '3px solid', ...(reg.paymentStatus === 'Approved' ? { background: '#d1fae5', borderColor: '#10b981' } : reg.paymentStatus === 'Rejected' ? { background: '#fee2e2', borderColor: '#ef4444' } : reg.paymentStatus === 'Pending' ? { background: '#fef3c7', borderColor: '#f59e0b' } : { background: '#dbeafe', borderColor: '#3b82f6' }) }}>
                        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '1rem', flexWrap: 'wrap', gap: '0.5rem' }}>
                          <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                            <span style={{ fontSize: '1.5rem' }}>💳</span>
                            <span style={{ fontWeight: '700', fontSize: '1rem' }}>Payment Status</span>
                          </div>
                          <span style={{ ...styles.badge(reg.paymentStatus === 'Approved' ? 'registered' : reg.paymentStatus === 'Pending' ? 'waitlisted' : 'cancelled'), padding: '0.5rem 1rem', fontSize: '0.875rem' }}>
                            {reg.paymentStatus === 'Approved' ? '✅ Approved' : reg.paymentStatus === 'Rejected' ? '❌ Rejected' : reg.paymentStatus === 'Pending' ? '⏳ Pending Review' : '📝 Upload Required'}
                          </span>
                        </div>
                        
                        {/* Status-specific Messages */}
                        {reg.paymentStatus === 'Approved' && (
                          <div style={{ background: 'white', borderRadius: '0.5rem', padding: '1rem', border: '2px solid #10b981' }}>
                            <div style={{ display: 'flex', gap: '0.75rem', alignItems: 'start' }}>
                              <span style={{ fontSize: '2rem' }}>🎉</span>
                              <div>
                                <p style={{ fontWeight: 'bold', color: '#059669', marginBottom: '0.5rem' }}>Payment Approved!</p>
                                <p style={{ fontSize: '0.875rem', color: '#065f46', lineHeight: '1.5' }}>
                                  Your payment has been verified and approved by the organizer. Your ticket and QR code are now available above. You can view and download your ticket anytime.
                                </p>
                              </div>
                            </div>
                          </div>
                        ...
                        {reg.paymentStatus === 'Pending' && (
                          <div style={{ background: 'white', borderRadius: '0.5rem', padding: '1rem', border: '2px solid #f59e0b' }}>
                            <div style={{ display: 'flex', gap: '0.75rem', alignItems: 'start' }}>
                              <span style={{ fontSize: '2rem' }}>⏳</span>
                              <div>
                                <p style={{ fontWeight: 'bold', color: '#d97706', marginBottom: '0.5rem' }}>Payment Under Review</p>
                                <p style={{ fontSize: '0.875rem', color: '#92400e', lineHeight: '1.5' }}>
                                  Your payment proof has been submitted and is currently being reviewed by the organizer. You'll receive your ticket and QR code once the payment is approved. This usually takes 24-48 hours.
                                </p>
                                {reg.paymentProof && (
                                  <div style={{ marginTop: '1rem' }}>
                                    <p style={{ fontSize: '0.75rem', color: '#78350f', fontWeight: 'bold', marginBottom: '0.5rem' }}>Submitted Payment Proof:</p>
                                    <SecureImage
                                      ...
                                      onClick={async () => {
                                        try {
                                          const response = await api.get(`/registrations/${reg._id}/payment-proof-url`);
                        ...
                        {reg.paymentStatus === 'Rejected' && (
                          <div style={{ background: 'white', borderRadius: '0.5rem', padding: '1rem', border: '2px solid #ef4444', marginBottom: '1rem' }}>
                            <div style={{ display: 'flex', gap: '0.75rem', alignItems: 'start' }}>
                              <span style={{ fontSize: '2rem' }}>❌</span>
                              <div>
                                <p style={{ fontWeight: 'bold', color: '#dc2626', marginBottom: '0.5rem' }}>Payment Rejected</p>
                                <p style={{ fontSize: '0.875rem', color: '#991b1b', lineHeight: '1.5', marginBottom: '0.5rem' }}>
                                  Your payment proof was not accepted. Please upload a new payment proof with clear transaction details.
                                </p>
                                {reg.paymentRemarks && (
                                  <div style={{ background: '#fef2f2', padding: '0.75rem', borderRadius: '0.5rem', marginTop: '0.5rem' }}>
                                    <p style={{ fontSize: '0.75rem', fontWeight: 'bold', color: '#991b1b', marginBottom: '0.25rem' }}>Organizer's Feedback:</p>
                                    <p style={{ fontSize: '0.875rem', color: '#7f1d1d' }}>{reg.paymentRemarks}</p>
                                  </div>
                                )}
                              </div>
                            </div>
                        ...
                        {(!reg.paymentStatus || reg.paymentStatus === 'Rejected') && (
                          <div style={{ background: 'white', borderRadius: '0.5rem', padding: '1rem', border: '2px solid #3b82f6', marginBottom: '1rem' }}>
                            <div style={{ display: 'flex', gap: '0.75rem', alignItems: 'start' }}>
                              <span style={{ fontSize: '2rem' }}>📸</span>
                              <div style={{ flex: 1 }}>
                                <p style={{ fontWeight: 'bold', color: '#1e40af', marginBottom: '0.5rem' }}>Upload Payment Proof Required</p>
                                <p style={{ fontSize: '0.875rem', color: '#1e3a8a', lineHeight: '1.5', marginBottom: '0.75rem' }}>
                                  To receive your merchandise ticket and QR code, please upload a clear screenshot or photo of your payment transaction showing:
                                </p>
                                <ul style={{ fontSize: '0.875rem', color: '#1e3a8a', marginLeft: '1.5rem', marginBottom: '0.75rem' }}>
                                  <li>Transaction ID or reference number</li>
                                  <li>Payment amount and date</li>
                                  <li>Recipient details</li>
                                </ul>
                                <p style={{ fontSize: '0.75rem', color: '#3730a3', fontStyle: 'italic' }}>
                                  💡 Tip: Ensure the image is clear and all details are visible for quick approval.
                                </p>
                              </div>
                            </div>
                          </div>
                    ...
                    <div style={{ display: 'flex', gap: '0.75rem', flexWrap: 'wrap' }}>
                      ...
                      {/* Only show View Ticket button if QR code exists (payment approved for merchandise) */}
                      {reg.qrCode && (
